...
const PointOfSale = () => {
  ...
  useEffect(() => {
    if (customer.mobile && customer.mobile.length >= 10) {
      const existingCustomer = getCustomerByMobile(customer.mobile);
      if (existingCustomer) {
  ...
  const handleMobileSearch = () => {
    if (mobileSearch.trim()) {
      const existingCustomer = getCustomerByMobile(mobileSearch);
      if (existingCustomer) {
        setCustomer(existingCustomer);
        setMobileSearch("");
      } else {
  ...
  const handlePaymentSuccess = (paymentDetails) => {
    // Save customer details automatically during checkout
    ...
    localStorage.setItem("localshop_products", JSON.stringify(updatedProducts));
  ...
  const handleCheckout = () => {
    ...
    if (paymentMethod === "CASH") {
      // For cash payments, process directly without payment gateway
      handlePaymentSuccess({
        method: "CASH",
        transactionId: `CASH${Date.now()}`,
      });
    } else {
      // For other payment methods, show payment gateway
